...
func _update_counter_label() -> void:
	var lbl: Label = get_node_or_null("CounterLabel") as Label
	if lbl:
...
func _draw_portals() -> void:
	for portal_pos in [_portal_a_pos, _portal_b_pos]:
		var is_entry: bool = (portal_pos == _portal_a_pos)
		var label_text: String = "入口 A" if is_entry else "出口 B"
		var col: Color
		var alpha: float
	...
	# 虫洞连线
	if _wormhole_stable:
		var mid: Vector2 = (_portal_a_pos + _portal_b_pos) / 2.0
		var perp: Vector2 = (_portal_b_pos - _portal_a_pos).orthogonal().normalized() * 40.0
		var ctrl: Vector2 = mid + perp
		var prev_pt: Vector2 = _portal_a_pos
		for i in range(1, 21):
			var t_param: float = i / 20.0
			var pt: Vector2 = _portal_a_pos.bezier_interpolate(ctrl, ctrl, _portal_b_pos, t_param)
			draw_line(prev_pt, pt, Color(C_CYAN.r, C_CYAN.g, C_CYAN.b, 0.3 + sin(_t * 3.0 + t_param) * 0.15), 2.0, true)
			prev_pt = pt


func _draw_casimir_plates() -> void:
	var plates: Array = [{"x": _plate_a_x, "label": "板 A"}, {"x": _plate_b_x, "label": "板 B"}]
	for p in plates:
		var px: float = p["x"]
		var rect: Rect2 = Rect2(px - PLATE_SIZE.x / 2.0, _plate_y - PLATE_SIZE.y / 2.0, PLATE_SIZE.x, PLATE_SIZE.y)
