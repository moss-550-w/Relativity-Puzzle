# 《时空错位：相对论漫游》完整设计文档

**版本：v1.0-pre**
**类型：2D 横版解谜 / 轻平台跳跃**
**引擎：Godot 4.4+（GDScript）**
**目标平台：Windows / macOS / Linux PC**
**体量：4 个世界，11 个关卡 + 3 个小游戏 + 2 个彩蛋，约 3–4 小时流程**

***

## 目录

1. [设计铁律](#一设计铁律)
2. [主角与操作](#二主角与操作)
3. [世界 1：洛伦兹平原](#三世界-1洛伦兹平原)
4. [世界 2：引力深渊](#四世界-2引力深渊)
5. [世界 3：量子泡沫秘境](#五世界-3量子泡沫秘境)
6. [世界 4：熵之终焉](#六世界-4熵之终焉)
7. [全局系统](#七全局系统)
8. [技术架构](#八技术架构)
9. [美术与音效](#九美术与音效)
10. [结局系统](#十结局系统)
11. [趣味小游戏](#十一趣味小游戏)
12. [彩蛋](#十二彩蛋)
13. [开发状态](#十三开发状态)

***

## 一、设计铁律

| 编号     | 原则              | 说明                      |
| ------ | --------------- | ----------------------- |
| **P1** | **玩法 = 物理原理**   | 所有机制源自相对论真实结论，不编造科幻设定   |
| **P2** | **操作即时响应**      | 时间效应不影响玩家输入，通过画面/场景时钟表达 |
| **P3** | **失败无惩罚**       | 谜题重置即时，无死亡惩罚，无资源枯竭      |
| **P4** | **科普隐于玩法**      | 无大段文字，图鉴短句+动态示意，会玩就会懂   |
| **P5** | **无战斗、无血量、无升级** | 唯一成长是"理解"。能力由关卡机制赋予     |

***

## 二、主角与操作

| 输入         | 动作             | 获得时机                 |
| ---------- | -------------- | -------------------- |
| A/D 或 ← →  | 左右移动，长按累积加速    | 初始                   |
| W 或 空格     | 跳跃             | 初始                   |
| Left Shift | 冲刺（累积速率 ×3.5）  | 初始                   |
| **二段跳**    | 空中再次跳跃         | 克尔黑洞 BOSS 关（CP1）持久解锁 |
| F          | 抓取/拖拽（虫洞口、金属板） | World 2 起            |
| E          | 观测（延长量子裂隙 + NPC 交谈） | World 3 起            |
| R          | 重置谜题           | 全程可用                 |
| ESC        | 暂停菜单 / 返回       | 全程可用                 |

**速度累积系统**：长按方向键 → `ramped_speed` 持续增长（最高 980px/s ≈ 0.98c）→ 光速红线（990px/s = 0.99c）触发弹回。

***

## 三、世界 1：洛伦兹平原

**物理学**：狭义相对论 — 时间膨胀、光速壁垒、光锥、双生子佯谬
**状态**：✅ 全部实现

### M1-1：三重时间考验

| 要素         | 内容                                          |
| ---------- | ------------------------------------------- |
| **Zone 1** | 周期闸门 — 提速让闸门加速升降 → 乘隙穿过                     |
| **Zone 2** | 移动平台 — 提速让平台快进靠岸                            |
| **Zone 3** | 终点时空碎片                                      |
| **图鉴**     | `time_dilation`                             |
| **彩蛋**     | 向左走发现「双生子小屋」— 左侧 NPC 在高速区快速衰老 vs 右侧正常流速同框对比 |
| **图鉴(彩蛋)** | `twin_paradox`                              |

### M1-2：光速红线

| 要素     | 内容                            |
| ------ | ----------------------------- |
| **机制** | 红线壁垒阻断主路 → 高速冲撞弹回 → 乘升降台到上层绕过 |
| **图鉴** | `light_speed_barrier`         |

### M1-3：光锥边界

| 要素     | 内容                                |
| ------ | --------------------------------- |
| **机制** | 青蓝虚线 = 未来光锥安全区 → 走出即冻结 3 秒 → 自动重置 |
| **谜题** | 中途有闸门需加速通过，但不能冲出光锥边界              |
| **图鉴** | `light_cone`                      |

***

## 四、世界 2：引力深渊

**物理学**：广义相对论 — 引力时间膨胀、虫洞 CTC、克尔黑洞
**状态**：✅ 全部实现
**新能力**：二段跳（克尔黑洞 BOSS 关解锁，持久生效）

### M2-1：引力时间膨胀

| 要素     | 内容                                                               |
| ------ | ---------------------------------------------------------------- |
| **机制** | 紫色引力区覆盖快速升降平台。正常人无法通过（0.8s 周期）。进入引力区 → 场景减速至 0.25× → 平台变 3.2s 周期 |
| **教学** | 静止在引力区中让减速最大化——冲刺的 γ 会抵消引力效果                                     |
| **图鉴** | `gravitational_time_dilation`                                    |

### M2-2：闭合类时曲线（CTC）

| 要素       | 内容                                                                                          |
| -------- | ------------------------------------------------------------------------------------------- |
| **机制**   | 虫洞对（蓝色 A 固定、橙色 B 可 F 拖拽）。拖 B 入引力区 → 时间差激活 → 进 A 传送到 B + 进入"过去回响" → 门打开 → 按开关 → 返回现在 → 门保持开启 |
| **悖论检测** | 过去态下靠近传送前位置 → 白光悖论爆发 → 解锁祖父悖论图鉴 → 重置                                                        |
| **图鉴**   | `wormhole`、`ctc`、`grandfather_paradox`                                                      |

### BOSS：克尔黑洞

| 要素          | 内容                                                |
| ----------- | ------------------------------------------------- |
| **类型**      | 非战斗——极限环境穿越挑战                                     |
| **Phase 1** | 穿越能层 → 对抗帧拖拽 + 躲避公转碎片 → CP1（解锁二段跳）                |
| **Phase 2** | 内外视界间通道 → 内视界 5s 椭圆形变 → 时间泡等待                     |
| **Phase 3** | 奇环 16 段碰撞体 → 每 3s 开放 2 段间隙 → 穿越 → 核心              |
| **解体**      | 触碰奇环 → 18 像素粒子炸开 → 0.6s 后重生在最近检查点                 |
| **图鉴**      | `kerr_black_hole`、`ergosphere`、`ring_singularity` |

***

## 五、世界 3：量子泡沫秘境

**物理学**：量子引力 — 卡西米尔效应、真空涨落、时序保护
**状态**：✅ 全部实现
**新能力**：E 键观测（延长量子裂隙）

### M3-1：卡西米尔效应

| 要素     | 内容                                                                     |
| ------ | ---------------------------------------------------------------------- |
| **机制** | 封闭空间只能通过不稳定虫洞进入。两块金属板（F 键拖拽）推拢 < 100px → 板间产生蓝色负能量光柱 → 虫洞从橙闪变为蓝稳 → 可穿越 |
| **图鉴** | `casimir_effect`                                                       |

### M3-2：量子裂隙网络

| 要素      | 内容                                                           |
| ------- | ------------------------------------------------------------ |
| **机制**  | 三房间隔离，4 对量子裂隙交错出现。裂隙生命周期：预兆(0.5s)→打开(2s)→坍缩。E 键延长 +2s（冷却 3s） |
| **时间表** | F1(0s, A→B)、F2(2.5s, A→C直通)、F3(5s, B→C主线)、F4(7s, B→C备选)      |
| **图鉴**  | `quantum_vacuum`                                             |

### M3-3：时序保护

| 要素       | 内容                                                                |
| -------- | ----------------------------------------------------------------- |
| **机制**   | 三机制综合：负能量板稳定裂隙 + E 键观测延长 + 时序保护上限                                 |
| **时序保护** | 左上角 3 个菱形标记，每次 E 干预 +1。计数 ≥3 → 全屏金红粒子风暴 → 2s 后谜题重置。15s 未干预自动衰减 -1 |
| **最优解**  | 负能量稳定 F1（0 干预）→ E 延 F2（1 干预）→ 到达                                  |
| **图鉴**   | `chronology_protection`                                           |

***

## 六、世界 4：熵之终焉

**物理学**：热力学第二定律、块状宇宙、惠勒-德维特方程
**状态**：✅ 全部实现

### 上层：熵之斜坡

| 要素       | 内容                                                              |
| -------- | --------------------------------------------------------------- |
| **机制**   | 全局熵值 \[0,1] 线性递增（\~125s 到 1.0）。3 个熵增平台渐进崩解（完整→裂纹→崩解→消失），碰撞体同步缩小 |
| **秩序能量** | 3 个金色光点可收集，每次消耗回退熵 0.12（约 15s）。收集音效为上琶音（523→659→784Hz）         |
| **昼夜**   | 90s 周期正弦暖橙（昼）↔ 深蓝（夜）CanvasLayer 叠加                              |
| **图鉴**   | `entropy_arrow`                                                 |

### 中层：时间的碎片

| 要素       | 内容                                                                                               |
| -------- | ------------------------------------------------------------------------------------------------ |
| **机制**   | 6 个时区（绿 1×、红 0.05×、蓝 5×、紫 0.3×、金 3×、灰 0×）水平排列。进入时区 → `TimeManager.gravity_factor` 平滑 tween 到该区倍率 |
| **碎片**   | A(红区·极慢平台 5s 周期)、B(蓝区·极快平台 4s 周期)、C(灰区·完全冻结·靠惯性)                                                 |
| **熵增暂停** | 中层不受上层崩解影响                                                                                       |
| **图鉴**   | `block_universe`                                                                                 |

### 底层：本源静止空间

| 要素     | 内容                                                        |
| ------ | --------------------------------------------------------- |
| **机制** | 时间完全静止。3 组时间切片——同一物体以 5 个时间副本平铺（红\[过去]→白\[现在]→蓝\[未来]）     |
| **独白** | 4 段字幕分别于 10s/30s/60s/90s 淡入："时间不流动……惠勒-德维特方程中，时间消失了……"    |
| **音景** | 36Hz 极低频嗡鸣 + 第二泛音，时间静止时专属环境音                               |
| **结局** | 120s 后自动触发 `PlayerMetrics.determine_ending()` → 冻结 + 结局字幕 |
| **图鉴** | `wheeler_dewitt`（60s）、`time_illusion`（90s）                |
| **彩蛋** | 右侧远处发现「物理学家小屋」— 爱因斯坦与霍金 NPC 对话                           |
| **图鉴(彩蛋)** | `physicists_cabin`                                       |

***

## 七、全局系统

### Autoload 加载顺序

| 顺序 | 名称                | 职责                                    |
| -- | ----------------- | ------------------------------------- |
| 1  | `GameState`       | 世界/关卡索引、暂停、二段跳解锁、进度追踪、关卡注册表           |
| 2  | `TimeManager`     | 场景时间缩放（速度γ × 引力因子）、玩家实时 delta         |
| 3  | `CodexManager`    | 图鉴解锁、词条数据（23 条）                       |
| 4  | `AudioManager`    | 程序化音频合成（AudioStreamGenerator）— 动态 BGM（三层旋律）+ 10 种 SFX |
| 5  | `PlayerMetrics`   | 行为追踪：CTC 次数、熵减干预、底层静止时间、结局判定          |
| 6  | `EntropySystem`   | 全局熵值 \[0,1]、秩序资源、里程碑信号                |
| 7  | `TransitionLayer` | 场景过渡管理器：淡出(0.3s) → change_scene → 淡入(0.4s) |

### TimeManager 计算公式

```
scene_time_scale = γ (player_lorentz_factor)
gravity_factor   = 1.0 (默认) / 时区倍率 (中层) / 引力区倍率 (W2)
scaled_delta     = delta × scene_time_scale × gravity_factor
player_delta     = delta (始终实时)
```

### 关卡推进与选择

- 集中式 `LEVEL_REGISTRY` 定义全部 11 关卡元数据（名称、场景路径）
- `GameState` 追踪已完成关卡 + 自动解锁下一关（包括跨世界）
- 关卡选择界面：4 世界面板 + 关卡按钮（完成/解锁/锁定三态）+ 星空背景
- 关卡完成面板：下一关（自动跨世界）/ 重新挑战 / 返回选关
- 场景过渡：全屏淡入淡出，`TransitionLayer` 统一管理

### 图鉴词条

| 分类                  | ID                        | 中文名         | 解锁位置        |
| ------------------- | ------------------------- | ----------- | ----------- |
| special\_relativity | `time_dilation`           | 时间膨胀        | M1-1        |
| special\_relativity | `light_speed_barrier`     | 光速不变与光速壁垒   | M1-2        |
| special\_relativity | `light_clock`             | 光钟与时间膨胀     | 光钟工坊小游戏     |
| special\_relativity | `light_cone`              | 光锥与因果结构     | M1-3        |
| special\_relativity | `twin_paradox`            | 双生子佯谬       | M1-1 彩蛋     |
| general\_relativity | `gravitational_time_dilation` | 引力时间膨胀  | M2-1        |
| general\_relativity | `wormhole`                | 虫洞与爱因斯坦-罗森桥 | M2-2        |
| general\_relativity | `traversable_wormhole`    | 可穿越虫洞       | 虫洞工程师小游戏    |
| general\_relativity | `ctc`                     | 闭合类时曲线 CTC  | M2-2        |
| general\_relativity | `grandfather_paradox`     | 祖父悖论        | M2-2 悖论触发   |
| general\_relativity | `kerr_black_hole`         | 克尔黑洞        | BOSS CP1    |
| general\_relativity | `ergosphere`              | 能层与帧拖拽      | BOSS CP2    |
| general\_relativity | `ring_singularity`        | 奇环与裸奇点      | BOSS 核心     |
| quantum\_gravity    | `casimir_effect`          | 卡西米尔效应与负能量  | M3-1        |
| quantum\_gravity    | `quantum_vacuum`          | 量子真空涨落      | M3-2        |
| quantum\_gravity    | `chronology_protection`   | 时序保护猜想      | M3-3        |
| entropy\_cosmology  | `entropy_arrow`           | 熵增定律与时间箭头   | W4 上层       |
| entropy\_cosmology  | `block_universe`          | 块状宇宙        | W4 中层       |
| entropy\_cosmology  | `wheeler_dewitt`          | 惠勒-德维特方程    | W4 底层 60s   |
| entropy\_cosmology  | `time_illusion`           | 时间的主观性错觉    | W4 底层 90s   |
| entropy\_cosmology  | `maxwell_demon`           | 麦克斯韦妖        | 麦克斯韦妖小游戏    |
| entropy\_cosmology  | `landauer_principle`      | 兰道尔原理        | 麦克斯韦妖小游戏    |
| entropy\_cosmology  | `physicists_cabin`        | 物理学家的午后     | W4 底层彩蛋     |

***

## 八、技术架构

### 物理层

| 层 ID | 名称       | 用途        |
| ---- | -------- | --------- |
| 1    | world    | 地面、墙壁、障碍物 |
| 2    | player   | 玩家角色      |
| 3    | platform | 移动平台      |

### 项目文件结构（实际）

```
scripts/
├── palette.gd                     # 全局调色板（配色唯一真源 + 星空接口）
├── autoload/
│   ├── game_state.gd              # 全局状态 + 进度追踪 + 关卡注册表 + 小游戏注册表
│   ├── time_manager.gd            # 时间控制（γ × gravity_factor）
│   ├── codex_manager.gd           # 图鉴系统（23 词条）
│   ├── audio_manager.gd           # 程序化音频合成（动态 BGM 三层旋律 + 10 种 SFX + 环境音层）
│   ├── player_metrics.gd          # 行为追踪 + 结局判定
│   ├── entropy_system.gd          # 全局熵增（World 4）
│   └── transition_layer.gd        # 场景过渡管理器
├── player/
│   ├── player.gd                  # 移动 + 二段跳 + E 键观测
│   ├── player_abilities.gd        # 观测/抓取（桩）
│   └── player_glow.gd             # 速度光晕 + 技能闪色
├── mechanics/
│   ├── speed_time_coupling.gd     # 洛伦兹因子
│   ├── time_dilation.gd           # TimeAffectedBody 基类
│   ├── light_cone_boundary.gd     # 光锥安全区
│   ├── gravity_zone.gd            # 引力区
│   ├── wormhole_pair.gd           # 虫洞对 + CTC 状态机
│   ├── kerr_black_hole.gd         # 克尔黑洞核心
│   ├── casimir_plates.gd          # 卡西米尔板对
│   ├── quantum_fissure.gd         # 量子裂隙
│   ├── chronology_protection.gd   # 时序保护
│   ├── time_zone.gd               # 时区
│   └── time_slice.gd              # 时间切片
├── objects/
│   ├── moving_platform.gd         # 移动平台（受时间膨胀）
│   ├── cycled_gate.gd             # 周期闸门
│   ├── redline_barrier.gd         # 红线壁垒
│   ├── time_fragment.gd           # 时空碎片
│   ├── ctc_door.gd                # CTC 联动门
│   ├── unstable_wormhole.gd       # 不稳定虫洞
│   ├── entropy_platform.gd        # 熵增平台（含崩解音效触发）
│   └── order_energy.gd            # 秩序能量（含上琶音收集音效）
├── npc/
│   ├── twin_room.gd               # 双生子小屋
│   └── einstein_hawking_npc.gd    # 物理学家小屋（爱因斯坦 + 霍金 + 对话系统）
└── ui/
    ├── relative_clock.gd          # HUD
    ├── hud_gauge.gd               # 环形仪表
    ├── time_warp_overlay.gd       # 时间色偏 Shader
    ├── world_backdrop.gd          # 四世界差异化背景与网格
    └── codex_icon.gd              # 图鉴词条极简图标（23 词条 × 10 原型）

scenes/
├── player.tscn
├── world_1_lorentz/
│   ├── level_base.gd              # 关卡基类（过渡、完成面板、暂停菜单）
│   ├── level_1_1.gd/.tscn         # 三重时间考验 + 双生子彩蛋
│   ├── level_1_2.gd/.tscn         # 光速红线
│   └── level_1_3.gd/.tscn         # 光锥边界
├── world_2_gravity/
│   ├── level_2_1.gd/.tscn         # 引力时间膨胀
│   ├── level_2_2.gd/.tscn         # CTC 核心谜题
│   └── boss_ker_blackhole.gd/.tscn  # 克尔黑洞
├── world_3_quantum/
│   ├── level_3_1.gd/.tscn         # 卡西米尔效应
│   ├── level_3_2.gd/.tscn         # 量子裂隙网络
│   └── level_3_3.gd/.tscn         # 时序保护综合
├── world_4_entropy/
│   ├── level_4_upper.gd/.tscn     # 熵之斜坡
│   ├── level_4_middle.gd/.tscn    # 时间的碎片
│   └── level_4_bottom.gd/.tscn    # 本源静止空间 + 物理学家小屋彩蛋
├── ui/
│   ├── main_menu.gd/.tscn         # 主菜单（含图鉴面板）
│   ├── level_select.gd/.tscn      # 关卡选择界面
│   └── hud.tscn
└── minigames/
    ├── light_clock.gd/.tscn        # 光钟工坊
    ├── wormhole_engineer.gd/.tscn  # 虫洞工程师
    └── maxwell_demon.gd/.tscn      # 麦克斯韦妖
```

***

## 九、美术与音效

### 视觉效果速查

| 效果     | 视觉实现                         |
| ------ | ---------------------------- |
| 世界背景   | `world_backdrop.gd`：四世界差异化渐变 + 统一星空 + 坐标参考网格（平直/弯曲/破碎/衰败） |
| 调色板    | `palette.gd`：UI 色/世界色卡/分类色/星空接口的全局唯一真源 |
| 时间加速   | `time_warp.gdshader` v2 半透明蓝移色洗 |
| 时间减速   | `time_warp.gdshader` v2 半透明红移色洗 |
| 光速红线   | 全屏红移闪光 + 边框脉冲                |
| 运动尺缩   | 玩家 VisualRoot 横向压缩           |
| 速度光晕   | `player_glow.gd`：低速微白 → 0.5c 蓝晕 → 红线临界红脉冲 |
| 技能闪色   | E 观测青闪 / F 抓取橙闪（0.25s 衰减）     |
| 引力区    | 暗紫同心椭圆光环 + 粒子公转              |
| 虫洞     | 蓝/橙旋转光环 + 贝塞尔连线              |
| 克尔黑洞   | 四层光环 + 40 粒子 + 奇环间隙          |
| 卡西米尔   | 板间蓝色光柱 + 蓝边框辉光               |
| 量子裂隙   | 蓝紫光环 + 粒子汇聚/飘浮               |
| 时序风暴   | 全屏金→红闪光 + 30 高速粒子            |
| 昼夜循环   | CanvasLayer 暖橙↔深蓝 90s 周期     |
| 熵增崩解   | 平台色变灰褐 + 4 条裂缝渐显 + 噪音爆裂崩解    |
| 时间切片   | 5 色渐变排列（红→白→蓝）               |
| 光钟可视化  | 双参考系对比：垂直反弹 vs 锯齿斜线 + 直角三角形辅助线 |
| 虫洞工程师  | 虫洞闪烁橙/稳定蓝 + 粒子拖尾 + 目标区虚线框     |
| 星空背景   | 主菜单/关卡选择/小游戏统一星空接口，**Palette 集中管理** |
| 图鉴图标   | `codex_icon.gd`：23 词条 × 10 种几何原型，分类分色（时钟/光锥/虫洞/黑洞/…） |
| 暂停/完成面板 | 描边/阴影/标题随世界 accent 色自动切换           |
| NPC 人物  | Polygon2D 构造（爱因斯坦白爆发+西装 / 霍金轮椅+眼镜+绿屏） |

### 背景音乐（程序化动态合成）

BGM 由 `AudioManager` 用 `AudioStreamGenerator` **实时合成**，无任何外部音频资源。三层结构叠加成完整乐句，并按 `GameState.current_world` 自动切换调式与情绪——契合「玩法即物理」准则：**音乐本身也由物理状态驱动**。

**三层合成结构**

| 层 | 实现 | 作用 |
| --- | --- | --- |
| 主旋律 | 16 分音符音序器，每世界 4 段 pattern（陈述→应答→发展→收束）按小节轮播；三角波/正弦波 + 拨奏式包络 | 乐句旋律 |
| 低音 Bass | 每小节首拍触发，偶数小节根音 / 奇数小节五度交替，缓慢衰减 | 和声进行根基 |
| 和声 Pad | 持续柔和正弦三音和弦 + 慢速颤音 LFO | 音乐铺底（取代旧单频嗡鸣） |

**四世界调式 / 情绪**

| 世界 | 调式 | BPM | 主旋律波形 | 情绪 |
| --- | --- | --- | --- | --- |
| W1 洛伦兹平原 | A 大调五声 | 104 | 三角波 | 明亮轻快 |
| W2 引力深渊 | A 小调五声 | 72 | 正弦 | 低沉迟缓 |
| W3 量子泡沫 | C Lydian（含 #4 色彩音） | 92 | 三角波 | 飘忽迷离 |
| W4 熵之终焉 | D 全音阶 | 60 | 正弦 | 悬浮崩解 |

**物理状态联动**

| 物理量 | 来源 | 对音乐的影响 |
| --- | --- | --- |
| 速度 | `TimeManager.scene_time_scale` | ↑ → tempo 加快（跑得越快越急促） |
| 引力 | `TimeManager.gravity_factor` | ↓ → tempo 减慢（黑洞旁迟缓拖沓） |
| 熵 | `EntropySystem.global_entropy` | ↑ → 旋律随机丢音 + 失谐走音 + Pad 渐隐 + 混入粉噪（听觉化的时空崩坏） |

> 实现要点：AudioManager 设为 `PROCESS_MODE_ALWAYS`，暂停时持续填充缓冲防生成器欠载爆音；暂停期间音乐层静音、SFX 仍可播放（如 UI 确认音）。旋律 pattern 存于 `_MUSIC_PATTERNS` 常量表，改旋律只需调数组、无需动合成逻辑。

### 音效速查

| 事件               | 音频                                                | SFX ID              |
| ------------------ | ------------------------------------------------- | ------------------- |
| 背景音乐底床       | 和声 Pad 持续三音和弦 + 慢速颤音，按世界切换调式              | —（BGM 层）             |
| World 4 底层嗡鸣   | 36Hz 纯正弦，仅熵之终焉世界专属环境音                       | —（BGM 层）             |
| 熵崩坏失谐         | 高熵时旋律随机丢音 + 整体失谐 + 混入粉噪                     | —（BGM 层）             |
| 红线警告           | 高频谐波叠加（`redline_ratio` > 0.3），接近光速时更尖锐              | —（环境层）              |
| 红线弹回           | 300Hz→60Hz 扫频下降                                   | `redline_bounce`   |
| 碎片收集           | 440Hz→880Hz 上扬                                    | `fragment_collect` |
| 跳跃               | 220Hz→330Hz 短促                                    | `jump`             |
| 菜单悬停           | 800Hz 极短叮咚 (0.04s)                                 | `ui_tick`          |
| 菜单确认           | 440→660Hz 上扫 (0.15s)                                | `ui_confirm`       |
| 虫洞稳定化         | 60→180Hz 低频嗡鸣上升 (0.5s)                             | `wormhole_stabilize`|
| 熵平台崩解         | 噪音爆发 200→40Hz 下降 (0.4s)                            | `platform_crumble` |
| 量子裂隙预兆       | 300→600Hz 脉冲上升 (0.5s)                               | `fissure_warning`  |
| 光钟滴答           | 1200Hz 极短敲击 (0.03s)                                 | `photon_tick`      |
| 秩序能量收集       | 三音上琶音 523→659→784Hz (0.35s)                       | `order_collect`    |

***

## 十、结局系统

结局由 `PlayerMetrics` 追踪的行为统计在底层 120s 后自动判定：

| 结局       | 触发条件                         | 字幕                       |
| -------- | ---------------------------- | ------------------------ |
| **奔赴未来** | 默认（修复碎片 ≥ 3，CTC < 3，未满足其他条件） | "你选择向前。未来永远是开放的。"        |
| **闭环囚笼** | CTC 回溯 ≥ 3 次                 | "你执着于回溯过去，却被困在了因果的循环之中。" |
| **本源顿悟** | 底层静止 > 60s 且熵减干预 < 3         | "时间从未流动。你终于理解了。"         |

***

## 十一、趣味小游戏

三个小游戏从关卡选择界面进入，短时体验（2-5 分钟），完成解锁专属图鉴。

> 注：图鉴词条总数已达 23 条；麦克斯韦妖一次解锁 2 条。

### 光钟工坊

| 要素       | 内容                                                                |
| -------- | ----------------------------------------------------------------- |
| **物理原理** | 爱因斯坦光钟思想实验：运动 → 光子走斜边 → 滴答变长 → 时间膨胀 = γ                            |
| **操作**   | ← → 调速（连续 + 惯性减速）                                                   |
| **视觉**   | 左面板静止参考系（垂直反弹）+ 右面板实验室参考系（锯齿斜线 + 直角三角形辅助线 + 速度箭头）                     |
| **HUD**   | 环形速度仪表 + γ 因子 + 速度 %c + 静止/运动钟滴答计数对比                                |
| **完成条件** | 观察 ≥ 30 秒                                                          |
| **图鉴**   | `light_clock`                                                     |

### 虫洞工程师

| 要素       | 内容                                                               |
| -------- | ---------------------------------------------------------------- |
| **物理原理** | 可穿越虫洞需要负能量（卡西米尔效应）维持打开，否则坍缩                                       |
| **操作**   | F 键拖拽虫洞口（A 入口 / B 出口）和卡西米尔金属板                                      |
| **机制**   | 推拢板 → 间距 < 80px → 负能量区 → 虫洞口在范围内 → 稳定开启（蓝）→ 信号粒子穿越 → 到达目标区        |
| **视觉**   | 虫洞口闪烁橙（不稳）/ 稳定蓝 + 贝塞尔连线 + 卡西米尔板 + 负能量蓝光区 + 粒子拖尾 + 目标区虚线框             |
| **完成条件** | 成功运输 3 个信号粒子到目标区                                                  |
| **图鉴**   | `traversable_wormhole`                                           |

### 麦克斯韦妖

| 要素       | 内容                                                                              |
| -------- | ------------------------------------------------------------------------------- |
| **物理原理** | 麦克斯韦妖思想实验 + 兰道尔原理：擦除 1 bit 信息至少耗散 kT·ln2 热量，故「气体+小妖」总熵从不下降，第二定律不可违背         |
| **操作**   | 鼠标对准闸门高度；按住 空格/左键 开门分拣；E/Shift 擦除记忆；R 重置                                       |
| **机制**   | 闸门放行热(红)分子→右腔、冷(蓝)分子→左腔；每放行 1 分子耗 1 bit 记忆（共 8 bit），写满须擦除；擦除使全局熵阶跃 +0.08      |
| **视觉**   | 密闭气体箱（左冷腔/右热腔）+ 中央闸门 + 速度着色分子与拖尾 + 双计量条（局部有序度↑↓ / 全局熵单调↑）+ 记忆比特格              |
| **完成条件** | 局部有序度（分离度）≥ 70%                                                                 |
| **图鉴**   | `maxwell_demon`、`landauer_principle`                                            |
| **实现注意** | 使用独立的局部熵计量，刻意不写入共享 `EntropySystem`，避免污染世界4正式关卡的熵状态                              |

***

## 十二、彩蛋

| 彩蛋            | 位置     | 内容                                                  | 图鉴             |
| ------------- | ------ | --------------------------------------------------- | -------------- |
| 双生子小屋         | M1-1 左侧 | 程序化房间，左 NPC 15× 倍速衰老 vs 右 NPC 正常流速。站立观察 2s 解锁       | `twin_paradox` |
| 物理学家小屋        | W4 底层右侧 | 爱因斯坦（白爆发+胡须+西装）+ 霍金（轮椅+眼镜+绿屏）。E 键对话，每人 3 句循环名言。 | `physicists_cabin` |

物理学家对话内容：

**爱因斯坦**：
1. "引力不是力，是时空的弯曲。我的场方程 Rμν - ½gμνR = 8πGTμν 说的就是这个。"
2. "想象力比知识更重要。知识有限，想象力环绕世界。"
3. "关于量子理论，我可能错了。'上帝不掷骰子'——但也许祂确实掷。"

**霍金**：
1. "我提出了时序保护猜想：物理定律禁止宏观时间旅行。大自然讨厌时间机器。"
2. "记得仰望星空，而不是低头看脚下。试着理解你所看到的，思索宇宙为何存在。"
3. "黑洞不是完全黑的——它们会辐射。这是我最骄傲的发现。"

***

## 十三、开发状态

### ✅ Phase 1：核心原型（世界 1）— 完成

- [x] 速度-时间耦合 + 累积加速
- [x] M1-1（三重时间考验）+ 双生子小屋彩蛋
- [x] M1-2（光速红线壁垒）
- [x] M1-3（光锥边界教学关）
- [x] 基础图鉴框架（4 词条 → 扩展至 21 词条）
- [x] 程序化音频合成
- [x] HUD 系统（环形仪表 + γ 因子 + 时间流速条）

### ✅ Phase 2：机制扩展（世界 2+3）— 完成

- [x] 世界 2：引力时间膨胀 + CTC 虫洞 + 克尔黑洞 BOSS
- [x] 世界 3：卡西米尔板 + 量子裂隙 + 时序保护
- [x] 二段跳持久能力
- [x] E 键观测 + F 键拖拽

### ✅ Phase 3：内容填充与思辨关（世界 4）— 完成

- [x] 上层：熵增崩解 + 秩序能量 + 昼夜循环
- [x] 中层：多时区独立时间流速
- [x] 底层：时间切片 + 独白叙事 + 结局触发
- [x] 23 词条完整图鉴

### ✅ Phase 4：打磨与 UI — 完成

- [x] 主菜单场景（含图鉴面板：分类筛选 + 进度条 + **词条极简图标**）
- [x] 暂停菜单（继续 / 重新开始 / 返回选关 + ESC 关闭 + **描边随世界配色**）
- [x] 关卡选择界面（4 世界面板 + 11 关卡按钮三态 + 3 小游戏入口）
- [x] 场景过渡系统（TransitionLayer autoload，淡入淡出 0.3/0.4s）
- [x] 关卡完成面板（下一关自动跨世界 / 重新挑战 / 返回选关 + **描边随世界配色**）
- [x] **全局调色板 Palette（UI 色/世界色卡/分类色/星空接口唯一真源，修复多处不一致）**
- [x] **四世界差异化背景（world_backdrop.gd：纵向渐变 + 统一星空 + 坐标网格 W1 平直 / W2 弯曲 / W3 破碎 / W4 衰败）**
- [x] **time_warp.gdshader v2（白屏→半透明红/蓝色洗，静止时透出世界背景）**
- [x] **玩家速度光晕（player_glow.gd：低速微白 → 0.5c 蓝晕 → 红线红脉冲）+ E 青闪 / F 橙闪**
- [x] 趣味小游戏：光钟工坊（双参考系光子动画 + 滴答对比 + 30s 观察解锁）
- [x] 趣味小游戏：虫洞工程师（拖拽虫洞口/板 + 负能量稳定 + 粒子传送 3 次）
- [x] 趣味小游戏：麦克斯韦妖（分子分拣 + 有限记忆 + 兰道尔擦除代价 + 局部有序度 ≥ 70% 解锁图鉴）
- [x] 爱因斯坦/霍金 NPC 彩蛋（Polygon2D 人物 + E 键对话系统 + 循环名言）
- [x] 最终音效打磨（10 种 SFX + 环境嗡鸣 + 熵噪音退化 + 软限幅防削波）
- [x] **动态背景音乐（三层程序化合成：音序器主旋律 + 低音 + 和声 Pad；四世界调式切换；速度/引力/熵驱动节奏与崩解）**

### 全部完成 ✅

项目 v1.0-pre 所有规划功能已实现完毕。
